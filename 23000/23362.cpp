...
ll pollard_rho(ll num){ // finding divisor
    if(num%2==0) return 2;
...
vi PRP(ll num){ // find all divisor
    vi prp;
    while(num>1){
...
void dfs(ll x, ll s){
    ...
    forf(i, 0, divis[x].size()){
        dfs(x+1, s*divis[x][i]);
    }
}

void divs(ll num){
    vi prp=PRP(num); prp.pb(0); sortv(prp);
    vi v; v.pb(1);
    forf(i, 1, prp.size()){
        if(prp[i]!=prp[i-1]){
            divis.pb(v);
        }
        ll dvss=divis.size()-1;
        divis[dvss].pb(divis[dvss][divis[dvss].size()-1]*prp[i]);
    }
...
int main() {
	...
	while(t--){
        ll n, ans=0; cin >> n;
        divis.resize(0); divisor.resize(0);
        divs(n);
        sortv(divisor);
        forf(i, 0, divisor.size()) ans+=EP(divisor[i]+1);
        cout << ans << "\n";
	}
	return 0;
}
