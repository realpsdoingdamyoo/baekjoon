...
void solve_sudoku(vector<vector<int>> su){
    ...
    for(int i : ret){
        int x=i/81, y=i%81/9, z=i%9;
        if(su[x][y]==0) su[x][y]=z+1;
    }

    for(int i=0; i<9; i++) {for(int j=0; j<9; j++) cout << su[i][j] << " ";
    cout << "\n";}
...
signed main(){
    ...
    while(true){
        ...
        for(int i=0; i<81; i++){
            if(i%9==0) su.push_back({});
            if(s[i]=='.') su.back().push_back(0);
            else su.back().push_back(s[i]-'0');
        }*/

        vector<vector<int>> su(9, vector<int>(9));
        for(vector<int>& i : su) for(int& j : i) cin >> j;
