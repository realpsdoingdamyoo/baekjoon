...
int sb(int t, int xs, int ys, int xe, int ye) {
    ...
    for (int i=xs; i<=xe; i++) for (int j=ys; j<=ye; j++) {
        if (s[t][i][j]=='R') pq.push(sb(t, xs, ys, i-1, ye)^sb(t, i+1, ys, xe, ye));
        else if (s[t][i][j]=='B') pq.push(sb(t, xs, ys, xe, j-1)^sb(t, xs, j+1, xe, ye));
        else if (s[t][i][j]=='G') pq.push(sb(t, xs, ys, i-1, j-1)^sb(t, xs, j+1, i-1, ye)^sb(t, i+1, ys, xe, j-1)^sb(t, i+1, j+1, xe, ye));
    }
...
signed main() {
    ...
    for (int i=0; i<n; i++) for (int j=0; j<n; j++) s[0][i][j]=s[1][i][j]='X';
    ...
    for (int i=0; i<n; i++) for (int j=0; j<n; j++) for (int t : {0, 1}) xc[t][i][j]=(s[t][i][j]=='X');
