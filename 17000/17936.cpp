...
#define sza(x) (sz[x].t + sz[x].c)
    ...
    void check_root() {
        if(reroot_stack.empty()) return;

        for(int i : reroot_stack)
            set_root_(i);
        reroot_stack.clear();
    }
    ...
    int root(int x) {
        ...
        access(x); splay_real(x);
        push(x);
        while(real[x][0]) x = real[x][0], push(x);
        splay_real(x);
        return x;
    }
    ...
    int chain(int x, int y) {
        ...
        access(y);
        splay_real(y);
        reroot_stack.push_back(rt);
        return y;
    }

    void reroot(int x) {
        reroot_stack.push_back(x);
    }
...
namespace init {
    ...
    void dfs2(int x, int p, std::vector<int>& ret) {
        ret[x] = p;
        for(int i = fst[x]; i; i = nxt[i]) {
            int y = tar[i];
            if(y == p) continue;
    ...
    void reset() {
        using namespace AAA;
        for(int i = 0; i < N; i++) {
            AAA::val[i] = add[i].c = add[i].t = sum[i].c = sum[i].t = T(0);
            par[i] = sz[i].c = sz[i].t
                = real[i][0] = real[i][1]
