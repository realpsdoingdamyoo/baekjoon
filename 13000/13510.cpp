...
struct segtree{
    ...
    ll mx(ll s, ll e){
        ...
        s+=1<<X; e+=1<<X;
        while(s<=e){
            if(s&1) ret=max(ar[s], ret);
            if(~e&1) ret=max(ar[e], ret);
            s=(s+1)/2; e=(e-1)/2;
        }
        return ret;
    }
...
ll query(ll a, ll b){
    ...
    while(tp[a]!=tp[b]){
        ...
        ret=max(ret, seg.mx(in[tp[a]], in[a]));
        a=par[tp[a]];
    }
    if(dep[a]>dep[b]) swap(a, b);
    ret=max(ret, seg.mx(in[a]+1, in[b]));
    return ret;
}
