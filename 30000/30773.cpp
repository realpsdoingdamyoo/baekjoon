...
int main() {
    ...
    ll ai=0, bi=n-1, i=0, j=0;
    while(ai<=bi && i<ans.length()){
        while(a[ai]!=ans[i]) ai++;
        while(a[bi]!=ans[i]) bi--;
        //cout << ai << " " << bi << "\n";
        i++; ai++; bi--;
    ...
    string r;
    while(ai<=bi){
        while(a[ai]!=ans[j]) r+=a[ai++];
        while(a[bi]!=ans[j]) r+=a[bi--];
        r+=ans[j];
        //cout << ai << " " << bi << "\n";
        if(ai==bi) break;
        j++; ai++; bi--;
    ...
    ll s=-a.length()+r.length()*2-(ll)(ai==bi);
    cout << s << "\n" << r;
    if(ai==bi) r=r.substr(0, r.length()-1);
    reverse(all(r));
    cout << r;
